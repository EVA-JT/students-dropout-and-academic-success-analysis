...
Para o prossegimento do item 2, escolhemos a coluna 'Nota de adimissao' para calcular: valor mínimo e o máximo valor, média, mediana, desvio padrão, os quartis, construir o histograma, boxplot e interpretar os resultados.
...
```{r}
hist(df$Nota.de.admissao, main = "Histograma da nota de adimissão", xlab = "Nota de adimissão", ylab = "Frequência", col = "mediumblue", xlim = c(95, 190), ylim = c(0, 900))
...
Para o prossegimento do item 3, escolhemos as coluna 'Gênero' e 'Resultado' para fazer uma análise bivariada com duas colunas categóricas escolhidas da base de dados,
...
Para o prossegimento do item 4, escolhemos as coluna 'Bolsista' e 'Media das notas do primeiro semestre' para fazer gerar um gráfico boxplot considerando as duas colunas e interpretar os resultados.
...
tabela
boxplot(Media.das.notas.do.primeiro.semestre ~ Bolsista, data = df, col = c("red", "green"), main = "Desempenho de Bolsistas vs Não Bolsistas", ylab = "Bolsista", xlab = "Média das notas do primeiro semestre", horizontal = TRUE)
```
...
Para o prossegimento do item 6, escolhemos as coluna 'Nota de adimissao' e 'Media das notas do primeiro semestre' para fazer gráfico de dispersão, a linha de tendencia dos dados, a correlação entre as duas variáveis e gerar gráfico de dispersão, mostrar a equação da reta ajustada e ocoeficiente de determinação.

```{r}
plot(df$Nota.de.admissao, df$Media.das.notas.do.primeiro.semestre, col = "mediumblue", pch = 19, xlab = "Notas de admissão", ylab = "Média das notas do primeiro período")
